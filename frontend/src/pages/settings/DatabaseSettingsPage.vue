...
const loadConfigs = async () => {
  ...
  error.value = ''
  try {
    const res = await api.getSettings('database')
...
<template>
  <div>
    <div class="dashboard-header">
      <div>
        <h1>{{ t('settings.databaseTitle') }}</h1>
        <p>{{ t('settings.databaseSubtitle') }}</p>
      </div>
    </div>
    ...
    <template v-else>
      <SettingsSection :title="t('settings.databaseConnections')" :description="t('settings.databaseConnectionsDesc')">
        <SettingItem
          :label="t('settings.databaseUrl')"
          :description="t('settings.databaseUrlDesc')"
          type="password"
          v-model="configs.database_url"
          :placeholder="'sqlite:///./devmatrix.db'"
        />
        <SettingItem
          :label="t('settings.redisUrl')"
          :description="t('settings.redisUrlDesc')"
          type="password"
          v-model="configs.redis_url"
          :placeholder="'redis://localhost:6379/0'"
        />
        <SettingItem
          :label="t('settings.temporalHost')"
          :description="t('settings.temporalHostDesc')"
          type="text"
          v-model="configs.temporal_host"
