...
async def pause_workflow(project_id: str, db: Session = Depends(get_db)):
    ...
    if state.status in (ProjectStatus.COMPLETED.value, ProjectStatus.FAILED.value):
...
async def resume_workflow(project_id: str, db: Session = Depends(get_db)):
    ...
    if state.status != "paused":
        raise HTTPException(status_code=400, detail=f"Can only resume from 'paused', current: '{state.status}'")
...
async def cancel_workflow(project_id: str, payload: Optional[LifecycleAction] = None, db: Session = Depends(get_db)):
    state = db.query(ProjectStateModel).filter(ProjectStateModel.project_id == project_id).first()
    if state is None:
        raise HTTPException(status_code=404, detail=f"Project {project_id} not found")
    if state.status == ProjectStatus.COMPLETED.value:
